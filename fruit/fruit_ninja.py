...
fruits = ['melon', 'orange', 'pomegranate', 'guava', 'zombie',]    #entities in the game

# initialize pygame and create window
WIDTH = 800
HEIGHT = 500
FPS = 15                                             #controls how often the gameDisplay should refresh.
pygame.init()
pygame.display.set_caption('BLADE MASTERS')
...
background = pygame.image.load('images\\backgound1.jpg')                      #game background
font = pygame.font.Font(os.path.join(os.getcwd(), 'comic.ttf'), 42)
score_text = font.render('Score : ' + str(score), True, (255, 255, 255))    #score display
lives_icon = pygame.image.load('images/red_lives.png')                    #images that shows remaining lives
...
def draw_text1(display, text, size, x, y, color):
    font = pygame.font.Font(font_name, size)
...
def show_rules():                                  #rules screen
    creditsdisplay=pygame.display.set_mode((WIDTH, HEIGHT)) 
    draw_text1(creditsdisplay, "BLADE MASTERS", 120, WIDTH/2, 50, RED)
    draw_text1(creditsdisplay, "RULES", 60, WIDTH/2, 240, GREEN)
    draw_text1(creditsdisplay, "1.", 50, 20, 300, RED)
    draw_text1(creditsdisplay, "YOU DON'T HAVE TO KILL ZOMBIES", 50, 340, 300, YELLOW)
    draw_text1(creditsdisplay, "2.", 50, 20, 340, RED)
    draw_text1(creditsdisplay, "IF YOU KILL 4 ZOMBIES GAME IS OVER", 50, 365, 340, YELLOW)
    draw_text1(creditsdisplay, "3.", 50, 20, 380, RED)
    draw_text1(creditsdisplay, "YOU HAVE TO KILL THE HUMANS", 50, 320, 380, YELLOW)
    draw_text1(creditsdisplay, "Press Space Bar to start the game", 55, 400, 450, RED)
    pygame.display.flip()
        
def show_credits():                     #credits screen
    creditsdisplay=pygame.display.set_mode((WIDTH, HEIGHT))
    draw_text1(creditsdisplay, "BLADE MASTERS", 120, WIDTH/2, 50, RED)
    draw_text1(creditsdisplay, "A GAME BY - ", 60, 150, 220, GREEN)
    draw_text1(creditsdisplay, "ADITYA TUTEJA", 40, 435, 320, YELLOW)
    draw_text1(creditsdisplay, "Press Space Bar to start the game", 55, 400, 450, RED)
...
def show_gameover_screen():
    ...
    draw_text1(gameDisplay, "BLADE  MASTERS", 120, WIDTH / 2, 50,ORCHID)

    if not game_over :
        draw_text1(gameDisplay,"Score : " + str(score), 70, WIDTH / 2, 200,YELLOW)
        draw_text1(gameDisplay,"GAME OVER!", 70,300, 300,RED)
        img2 = pygame.image.load('images/game_over.png')
        gameDisplay.blit(img2,(470,240))
    draw_text1(gameDisplay,"Press R to check the RULES",64,WIDTH/2,370,BLACK)
    draw_text1(gameDisplay,"Press C to check the CREDITS",64,WIDTH/2,410,BLACK)
    draw_text1(gameDisplay, "Press Space Bar to begin !", 64, WIDTH / 2, 450,BLACK)
    pygame.display.flip()
    waiting = True
    while waiting:
        clock.tick(FPS)
